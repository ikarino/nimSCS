...
import custom_types, unit, field

type Manager = ref object
...
proc saveOutput(m: Manager) =
  ...
  var orderOfKilledFriends: seq[int]
  ...
  let output = %* {
    "result": {
      ...
      "orderOfkilledfriends": orderOfkilledfriends
    },
    "exp": {
...
proc removeEnemy(m: Manager, enemy: Enemy) =
  ...
  m.enemys = m.enemys.filter(e => e != enemy)
